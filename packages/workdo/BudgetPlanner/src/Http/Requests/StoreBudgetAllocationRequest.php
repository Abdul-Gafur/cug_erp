...
class StoreBudgetAllocationRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // Quarterly distribution — each quarter is optional (zero-filled if omitted)
            // but when provided, the four values must sum to allocated_amount
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            $q1 = (float) ($this->q1_amount ?? 0);
            $q2 = (float) ($this->q2_amount ?? 0);
            $q3 = (float) ($this->q3_amount ?? 0);
            $q4 = (float) ($this->q4_amount ?? 0);
            $total = $q1 + $q2 + $q3 + $q4;
            $allocated = (float) ($this->allocated_amount ?? 0);

            // Only validate the sum when at least one quarterly figure is provided
