...
interface Category { id: number; name: string; default_useful_life_months: number; depreciation_method: string }
...
export default function Edit({ asset, categories, funds, errors = {} }: Props) {
    const { t } = useTranslation();

    const [form, setForm] = useState({
        asset_name: asset.asset_name,
        description: asset.description ?? '',
        fund_id: asset.fund_id?.toString() ?? '',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        setProcessing(true);
        router.put(route('account.fixed-assets.update', asset.id), form, {
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[
                ...
                { label: asset.asset_code, url: route('account.fixed-assets.show', asset.id) },
                { label: t('Edit') },
            ]}
            pageTitle={`${t('Edit Asset')} — ${asset.asset_code}`}
        >
            <Head title={t('Edit Asset')} />
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">{t('Asset Code')}</label>
                                    <Input value={asset.asset_code} disabled className="bg-gray-50" />
                                    <p className="text-xs text-gray-500 mt-1">{t('Asset code is assigned automatically and cannot be changed.')}</p>
                                </div>
                        ...
                        <Card>
                            <CardHeader><CardTitle>{t('Valuation')}</CardTitle></CardHeader>
                            <CardContent className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <Select value={form.depreciation_method}
                                            ...
                                            <SelectContent>
                                                <SelectItem value="straight_line">{t('Straight-Line (IPSAS 17)')}</SelectItem>
                                            </SelectContent>
                                        </Select>
                                    </div>
                                </div>
